...
import { X, Calendar, Camera, Check, ChevronLeft, Pencil } from 'lucide-react';
...
export default function HomeworkModal({
  homework, onClose, onToggle, onEdit,
}: HomeworkModalProps) {
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ backgroundColor: 'var(--tg-secondary-bg)', maxHeight: '88vh' }}
      >
        {/* Handle */}
        ...
        <div className="flex items-start gap-3 px-5 pt-3 pb-2 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0 pt-0.5">
            <span className="block text-[10px] font-bold uppercase tracking-widest mb-0.5" style={{ color: style.color }}>
              {homework.subject}
            </span>
            <p className="text-[15px] font-semibold leading-tight line-clamp-2" style={{ color: 'var(--tg-text)' }}>
          ...
          <button
            ...
            title="Редактировать"
          >
            <Pencil size={13} style={{ color: 'var(--tg-hint)' }} />
          </button>
        ...
        {/* Scrollable body */}
        <div
          className="flex-1 overflow-y-auto px-5 pt-1 pb-4"
          style={{ WebkitOverflowScrolling: 'touch' } as React.CSSProperties}
        >
          {/* Deadline row */}
          <div
            className="flex items-center gap-2.5 p-3 rounded-2xl mb-4"
            style={{ backgroundColor: overdue ? 'rgba(255,59,48,0.12)' : 'var(--tg-bg)' }}
          >
            <Calendar size={16} style={{ color: overdue ? '#ff3b30' : 'var(--tg-accent)', flexShrink: 0 }} />
            <div>
              <p className="text-[13px] font-semibold" style={{ color: overdue ? '#ff3b30' : 'var(--tg-text)' }}>
          ...
          <div className="mb-5">
            ...
            <p className="text-[14px] leading-relaxed whitespace-pre-wrap" style={{ color: 'var(--tg-text)' }}>
        ...
        >
          ...
          >
            {homework.isDone
              ? 'Снять отметку'
              : <><Check size={18} strokeWidth={2.5} />Отметить выполненным</>
            }
          </button>
        </div>
      </div>
